...
type AsyncProducer interface {
	...
	AsyncSendMessages(ctx context.Context, topic string, partition int32, messages []*common.Message) error
}
...
func (p *saramaAsyncProducer) AsyncSendMessages(ctx context.Context, topic string, partition int32, messages []*common.Message) error {
	for _, msg := range messages {
		if err := p.AsyncSend(ctx, topic, partition, msg.Key, msg.Value, msg.Callback); err != nil {
			return err
		}
	}
	return nil
