...
func (a *BatchEncoder) AppendRowChangedEvent(
	...
	if err != nil {
		log.Error("AppendRowChangedEvent: avro encoding value failed", zap.Error(err))
		return errors.Trace(err)
	}

	key, err := a.encodeKey(ctx, e, topic)
	if err != nil {
		log.Error("AppendRowChangedEvent: avro encoding key failed", zap.Error(err))
		return errors.Trace(err)
	}
...
func (a *BatchEncoder) encodeValue(
	ctx context.Context, e *model.RowChangedEvent, topic string,
) ([]byte, error) {
	if e.IsDelete() {
		return nil, nil
	}
