...
// eventGroup is the event of the mq worker.
...
type worker struct {
	...
	msgChan *chann.DrainableChann[eventGroup]
...
func newWorker(
	...
	w := &worker{
		changeFeedID:                      id,
		protocol:                          protocol,
		msgChan:                           chann.NewAutoDrainChann[eventGroup](),
...
func (w *worker) nonBatchEncodeRun(ctx context.Context) error {
	...
	for {
		...
		case events, ok := <-w.msgChan.Out():
			...
			if err := w.encoderGroup.AddEvents(ctx, events.key.Topic, events.key.Partition, events.events...); err != nil {
				return errors.Trace(err)
			}

			//if event.events.GetTableSinkState() != state.TableSinkSinking {
			...
			//	continue
			//}
			//if err := w.encoderGroup.AddEvents(ctx, event.key.Topic, event.key.Partition, event.events); err != nil {
			//	return errors.Trace(err)
			//}
		}
	}
...
func (w *worker) batchEncodeRun(ctx context.Context) (retErr error) {
	...
	for {
		...
		case events, ok := <-w.msgChan.Out():
			...
			//w.statistics.ObserveRows(events.events)
			if err := w.encoderGroup.AddEvents(ctx, events.key.Topic, events.key.Partition, events.events...); err != nil {
				return errors.Trace(err)
			}
			...
			//		}
			//		if msg.events != nil {
			//			w.statistics.ObserveRows(msg.events.Event)
			//			events[index] = msg
			//			index++
			//		}
		}
	...
	//eventsBuf := make([]eventGroup, flushBatchSize)
	//for {
	//	start := time.Now()
...
//func (w *worker) batch(
//	ctx context.Context, events []eventGroup, flushInterval time.Duration,
//) (int, error) {
//	index := 0
//	max := len(events)
...
//		if event.events.GetTableSinkState() != state.TableSinkSinking {
//			event.events.Callback()
//			log.Debug("Skip event of stopped table", zap.Any("event", event.events))
//			continue
//		}
//		if _, ok := partitionedRows[event.key]; !ok {
//			partitionedRows[event.key] = make([]*dmlsink.RowChangeCallbackableEvent, 0)
//		}
//		partitionedRows[event.key] = append(partitionedRows[event.key], event.events)
//	}
